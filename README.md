...
Webhook management is JSON-only for now. Webhook certificate upload, `ChatFullInfo`, channel post and standalone poll updates, concrete chat member variant structs, FSM, scenes, storage, dependency injection, and full Bot API coverage are not implemented yet. The Stage 88 audit tracks the exact remaining Bot API 9.6 gaps.
