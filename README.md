...
Webhook management is JSON-only for now. Webhook certificate upload, optional concrete chat member variant structs, FSM, scenes, storage, dependency injection, and final full Bot API coverage audit are not implemented yet. The Stage 88 audit tracks the exact remaining Bot API 9.6 gaps.
