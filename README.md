...
Set user emoji status or organization verification:

```go
ok, err := b.SetUserEmojiStatus(ctx, aigram.SetUserEmojiStatusParams{
    UserID:                   123456789,
    EmojiStatusCustomEmojiID: "custom-emoji-id",
})
if err != nil {
    return err
}
...
ok, err = b.VerifyChat(ctx, aigram.VerifyChatParams{
    ChatID:            aigram.ChatIDString("@example_channel"),
    CustomDescription: "Official support",
})
if err != nil {
    return err
}
...
`SetUserEmojiStatus`, `VerifyUser`, `VerifyChat`, `RemoveUserVerification`, and `RemoveChatVerification` change real user/chat status and are manual-only live operations. Use dedicated test users/chats where possible, and do not log custom emoji IDs from production data.
...
Send a poll or dice message. `SendPoll` also supports Bot API 9.6 poll fields such as `correct_option_ids`, `allows_revoting`, options controls, poll descriptions, and structured `InputPollOption` values via `OptionObjects`:

```go
pollMessage, err := b.SendPoll(ctx, aigram.SendPollParams{
    ChatID:   aigram.ChatIDInt(123456789),
    Question: "Pick one",
    OptionObjects: []aigram.InputPollOption{
        {Text: "A"},
        {Text: "B"},
    },
})
...
Service message metadata such as `UsersShared`, `ChatShared`, chat backgrounds, video chats, proximity alerts, auto-delete timers, giveaway service fields, and paid/direct message price changes is decoded from incoming `Message` payloads.
...
```go
checklistMessage, err := b.SendChecklist(ctx, aigram.SendChecklistParams{
    BusinessConnectionID: "business-connection-id",
    ChatID:               aigram.ChatIDInt(123456789),
    Checklist: aigram.InputChecklist{
        Title: "Launch checks",
        Tasks: []aigram.InputChecklistTask{
            {ID: 1, Text: "Verify staging"},
        },
    },
})
if err != nil {
...
fmt.Println(checklistMessage.MessageID)

ok, err := b.SendMessageDraft(ctx, aigram.SendMessageDraftParams{
    ChatID:  aigram.ChatIDInt(123456789),
    DraftID: 1,
    Text:    "Draft text",
})
...
fullInfo, err := b.GetChatFullInfo(ctx, aigram.GetChatParams{
    ChatID: aigram.ChatIDInt(123456789),
})
if err != nil {
    return err
}
fmt.Println("max reactions:", fullInfo.MaxReactionCount)
...
Chat member updates and chat boost updates can be routed through the dispatcher. `GetUserChatBoosts` reads boost metadata, while `SetChatMemberTag`, `BanChatSenderChat`, and `UnbanChatSenderChat` require admin rights and remain manual-only:

```go
boosts, err := b.GetUserChatBoosts(ctx, aigram.GetUserChatBoostsParams{
    ChatID: aigram.ChatIDInt(-1001234567890),
    UserID: 987654321,
})
if err != nil {
...
fmt.Println("boosts:", len(boosts.Boosts))

if err := dispatcher.OnChatBoostFunc(func(ctx context.Context, update aigram.Update) error {
    fmt.Println("boosted chat:", update.ChatBoost.Chat.ID)
    return nil
}); err != nil {
    return err
}
...
Subscription invite links use Telegram Stars and change real paid channel access. Keep live checks manual-only, use a disposable test channel, and do not log full invite links:

```go
subscriptionLink, err := b.CreateChatSubscriptionInviteLink(ctx, aigram.CreateChatSubscriptionInviteLinkParams{
    ChatID:             aigram.ChatIDString("@testchannel"),
    Name:               "paid-test",
...
fmt.Println("created subscription invite link")

editedSubscriptionLink, err := b.EditChatSubscriptionInviteLink(ctx, aigram.EditChatSubscriptionInviteLinkParams{
    ChatID:     aigram.ChatIDString("@testchannel"),
    InviteLink: subscriptionLink.InviteLink,
    Name:       "paid-test-edited",
})
if err != nil {
    return err
}
fmt.Println("edited subscription invite link:", editedSubscriptionLink.Name)
```
...
sent, err := b.AnswerWebAppQuery(ctx, aigram.AnswerWebAppQueryParams{
    ...
    Result: aigram.InlineArticle(
        "webapp-result",
        "Mini App result",
        aigram.InputText("Mini App response"),
    ),
})
if err != nil {
    return err
...
_, err = b.EditMessageLiveLocation(ctx, aigram.EditMessageLiveLocationParams{
    BusinessConnectionID: businessConnectionID,
    Target:               aigram.EditTargetChat(aigram.ChatIDInt(chatID), liveLocationMessageID),
    Latitude:             51.5,
    Longitude:            -0.12,
})
if err != nil {
...
Games support includes `SendGame`, `SetGameScore`, `GetGameHighScores`, `Message.game` decoding, and `callback_game` inline keyboard buttons. Games require setup in BotFather and remain manual-only for live testing:

```go
markup := telegram.NewInlineKeyboard(
    []telegram.InlineKeyboardButton{telegram.InlineButtonGame("Play")},
)

msg, err := b.SendGame(ctx, aigram.SendGameParams{
    ChatID:        aigram.ChatIDInt(chatID),
...
_, err = b.SetGameScore(ctx, aigram.SetGameScoreParams{
    UserID:    userID,
    Score:     100,
    ChatID:    aigram.ChatIDInt(chatID),
...
Telegram Passport support includes `Message.passport_data` decoding and `SetPassportDataErrors`. Passport payloads are sensitive: live testing is manual-only, and this library stage intentionally does not implement decryption helpers. Use only structural validation errors and avoid logging encrypted data or user documents:

```go
ok, err := b.SetPassportDataErrors(ctx, aigram.SetPassportDataErrorsParams{
    UserID: userID,
    Errors: []telegram.PassportElementError{
        telegram.PassportElementErrorDataField{
            ...
            DataHash:  "dummy-hash",
            Message:   "Please check the first name.",
        },
    },
})
if err != nil {
...
Managed Bots 9.6 support includes `User.can_manage_bots`, `KeyboardButtonRequestManagedBot`, `managed_bot_created` messages, `managed_bot` updates, prepared keyboard buttons, and token management methods. Managed bot tokens are sensitive; do not log returned values, and keep live checks manual-only:

```go
button, err := b.SavePreparedKeyboardButton(ctx, aigram.SavePreparedKeyboardButtonParams{
    UserID: 123456789,
    Button: aigram.KeyboardButtonManagedBot("Create test bot", aigram.KeyboardButtonRequestManagedBot{
        RequestID:         1,
        SuggestedName:     "Test Bot",
        SuggestedUsername: "test_managed_bot",
    }),
})
if err != nil {
    return err
}
fmt.Println("prepared button:", button.ID)

managedToken, err := b.GetManagedBotToken(ctx, aigram.GetManagedBotTokenParams{UserID: 987654321})
if err != nil {
    return err
}
...
`SetWebhook` uses JSON when no certificate is provided. For self-signed webhook certificates, pass `Certificate: aigram.FileUpload(...)`; Telegram requires an uploaded `InputFile`, so file IDs and URLs are rejected for this parameter. The certificate reader is consumed but not closed by ai-gram. Optional concrete chat member variant structs, FSM, scenes, storage, and dependency injection are not implemented yet.
